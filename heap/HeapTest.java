...
//		System.out.println(mh.b[1].value);
	...
	/**
	 * = a list of priorities in mh.b[0..mh.size-1], separated by ", " and delimited
	 * by "[", "]"
	 */
	...
	public void test01EnsureSpace1() {
		/*
		 * Fields of class Heap as well as method ensureSpace have access modifier
		 * protected, so they can be referenced from within the class, from other
		 * classes in the same package, and from a subclass. We made Heap final, so one
		 * cannot create subclasses of it.
		 *
		 * Method griesHeapifyMax (directly above) can create an empty heap, store array
		 * b in field b, fix field size, and also fix the map accordingly. The four
		 * methods above with gries in their name work like that.
		 *
		 * The code below creates a heap with 6 elements in it. The size and capacity of
		 * array hp.b are both 6. The loop checks that calling ensureSpace with size < 6
		 * does not change the size of the heap. The code after the loop checks that the
		 * capacity is doubled and that the right values are in the beginning of the new
		 * array.
		 */
	...
	public void test34InsertMin_BubbleUpDuplicatePriorities() {
		...
		Heap<Integer> mh2 = griesHeapifyMin(new Integer[] { 4, 2, 1 }, new double[] { 4.0, 4.0, 4.0 });
	 ...
	 * This checks the case: left child has bigger priority but right child has
	 * smaller one.<br>
	...
	@Order(64)
	/**
	 * Test that polling from an array of size 3 works where a bubble down is
	 * necessary.
	 */
