...
public final class Heap<T> {
	 ...
	 * b[0] is the root; <br>
	 * For k > 0, (k-1)/2 (using int division) is the index in b of the parent of
	 * b[k]<br>
	 * For k >= 0, 2k+1 and 2k+2 are the indexes in b of left and right children of
	 * b[k].<br>
	 *
	 ...
	 *
	 * 4. For k in 1..size-1, // child node have higher priority than parent <br>
	 ...
	 *
	 * 5. The keys of map are the values in b[0..size-1]. This implies that size =
	 * map.size(). //
	 *
	...
	protected void ensureSpace() {
		...
		// we can test this method without calling method insert, which is below.
	...
	public void insert(T v, double p) throws IllegalArgumentException {
		...
		// test10Insert, ensures that this method maintains fields b and map properly,
		...
		if (map.containsKey(v)) { // v == null is accepted
			throw new IllegalArgumentException();
		}
		ensureSpace();
		int index = map.size();
		b[index] = new Pair(v, p);
		map.put(v, index);
		size++;
	...
	/**
	 * If a value with priority p1 belongs above a value with priority p2 in the
	 * heap, <br>
	 * return 1.<br>
	...
	void bubbleDown(int h) {
		...
		if (h < 0 || size() <= h)
			return;
		int c = 2 * h + 2;
		// compare left and right, if left fail, choose left
		if ((c > size() - 1) || compareTo(c - 1, c) >= 0) {
			// System.out.println("c-1: " + b[c - 1].priority + "\tc: " + b[c].priority);
			c--;
		}
		// bubbleDown if current node fail
		while ((c < size()) && (compareTo(h, c) < 0)) {
			swap(c, h);
			h = c;
			c = 2 * h + 2;
			if ((c > size() - 1) || compareTo(c - 1, c) >= 0) {
	 ...
	 * <br>
	 * Expected time: logarithmic. Worst-case time: linear in the size of the
	 * heap.<br>
	 * Throw a NoSuchElementException if the heap is empty.
	 */
	public T poll() {
		// TODO 7: When this method correct, these testing procedure will find no
		// errors:
		...
		// deal with array b , hash map and int size
		T val = b[0].value;
		T valD = b[map.size() - 1].value;
	...
	public void changePriority(T v, double p) {
		...
		if (compareTo(index, parent) > 0) {
			bubbleUp(index);
		} else if (child1 < size() && compareTo(index, child1) < 0) {
			bubbleDown(index);
		} else if (child2 < size() && compareTo(index, child2) < 0) {
			bubbleDown(index);
		}
	}
	 ...
	 * This is necessary because generics and arrays don't inter-operate nicely.
	 * <br>
	 * A student in CS2110 would not be expected to know about the need for this
	 * <br>
